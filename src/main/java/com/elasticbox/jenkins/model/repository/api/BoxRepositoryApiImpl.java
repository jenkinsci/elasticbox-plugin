...
public class BoxRepositoryApiImpl implements BoxRepository {
    ...
    public List<AbstractBox> getNoPolicyAndNoApplicationBoxes(String workspace) throws RepositoryException {
        try {
            ...
            logger.log(
                    Level.SEVERE,
                    "There is an error retrieving boxes for this workspace: " + workspace + " from the API", e);

            throw new RepositoryException(
                    "Error retrieving no policies and no application boxes from API, workspace: " + workspace);
        }
     ...
     * @return the list of boxes that are not policy boxes
     */
    @Override
     ...
     * @return the existing policy boxes
     */
    @Override
    public List<PolicyBox> getCloudFormationPolicyBoxes(String workspace) throws RepositoryException {
        ...
        try {
            ...
            throw new RepositoryException(
                    "Error retrieving cloudformation policies boxes from API, workspace: " + workspace);
        }
    }

    /**
     * Returns the rest of policy boxes that are not cloud formation policy boxes.
     * @return policy boxes that are not cloud formation policy boxes
     */
    @Override
    public List<PolicyBox> getNoCloudFormationPolicyBoxes(String workspace) throws RepositoryException {
        ...
        try {
            ...
            throw new RepositoryException(
                    "Error retrieving no cloudformation policies boxes from API, workspace: " + workspace);
        }
    }
    ...
    public AbstractBox findBoxOrFirstByDefault(String workspace, final String box) throws RepositoryException {
        ...
        try {
            ...
            logger.log(Level.SEVERE,
                    "There is an error retrieving box:" + box + " for this workspace: " + workspace, e);

            throw new RepositoryException(
                    "There is an error retrieving box:" + box + " for this workspace: " + workspace);
        }
    }
